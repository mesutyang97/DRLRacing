...
from race_track import RaceTrackEnv
...
def build_rnn(x, h, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None, regularizer=None):
    ...
    #====================================================================================#
    # YOUR CODE HERE
    x_e = build_mlp(x, output_size, scope, n_layers, size, activation = activation)
    gru = tf.nn.rnn_cell.GRUCell(output_size, activation = activation)
    history_length = x_e.shape[1]
    ...
    def sample_trajectory(self, env, animate_this_episode, is_evaluation):
        ...
        while True:
            ...
            if steps < self.history:
                ...
                padding = np.zeros((self.history - steps, rowsize))

                data = np.copy(meta_obs[:steps])
