...
def main():
	...
	path_to_pickle = "graphicReplayData/{}.pkl".format(args.pklname)
	with open(path_to_pickle, 'rb') as handle:
		record_buff = pickle.load(handle)

	output_dir_name = "graphicReplay/{}".format(args.pklname)

	plot_path(record_buff, output_dir_name)



def plot_path(record_buff, output_dir_name):
	...
	length = record_buff['length']
	width = record_buff['width']
	env_window_w = record_buff['env_window_w']
	obs_window_w = record_buff['obs_window_w']
	data = record_buff['data']

	max_t = record_buff['lastTime']
	...
	for index in range(max_t - 1):
		location = data[index, 0:2]
		velocity = data[index, 2:4]
